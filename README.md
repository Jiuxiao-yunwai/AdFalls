...
- Media3 播放器资源池：同一时间只保留一个视频处于播放状态。
...
已完成阶段 1-4：MVVM 基础结构、Room 数据层、Room Flow + ViewModel StateFlow 状态同步，以及信息流体验完善。当前阶段继续验证 Media3 视频播放、暂停、静音、列表滑出暂停、详情页进入/返回同步和播放器生命周期。
